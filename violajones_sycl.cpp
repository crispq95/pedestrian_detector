...
void imgDotSquareSYCL(uint32_t *imgIn, uint32_t *imgOut, unsigned int height, unsigned int width)
{
	int irow = 0, icol = 0;
    

	myQueue.parallel_for<class imgDotSquare>(
			sycl::range<1>{height*width},[=]\
			(sycl::id<1> idx){
				int i = idx[0];
...
// 	}
// }
void imgCopySYCL(uint32_t *imgIn, double *imgOut, unsigned int height, unsigned int width)
{
	myQueue.parallel_for<class imgCopy>(
			sycl::range<1>{width*height},[=]\
			(sycl::id<1> idx){
				int i = idx[0];

				imgOut[i] = (double)imgIn[i];
			}).wait();
}
...
void computeIntegralImgRowSYCL(uint32_t *imgIn, uint32_t *imgOut, int width, unsigned int height)
{
	myQueue
...
void computeIntegralImgColSYCL(uint32_t *imgOut, unsigned int width, int height)
{
    myQueue
	   .parallel_for<class computeIntegralImgCOL>(
			sycl::range<1>{width},[=]\
			(sycl::id<1> idx){
				int col = idx[0]; 
...
void computeFeature(double *img, double *imgSq, CvHaarFeature *f, double *featVal, int irow, int icol, int height, int width, double scale, float scale_correction_factor, int real_height, int real_width)
{
	int nRects = 0;
	int col = 0;
	int row = 0;
...
int main( int argc, char** argv )
{
    ...
    float * goodcenterX_tmp=NULL;
	float * goodcenterY_tmp=NULL;
	uint32_t * goodRadius_tmp=NULL;
	...
	int dataSize_cascade = N_MAX_STAGES+N_MAX_STAGES*N_MAX_CLASSIFIERS+N_MAX_STAGES*N_MAX_CLASSIFIERS; 
    ...
    //sycl::malloc_device<CvHaarClassifierCascade>(dataSize_cascade, myQueue);	//mal ??
	...
	auto dev_imgInt_f = sycl::malloc_device<double>(height*width, myQueue);
	auto dev_imgSqInt_f = sycl::malloc_device<double>(height*width, myQueue);
	...
	goodRadius_tmp=alloc_1d_uint32_t(N_MAX_STAGES*NB_MAX_DETECTION);
	...
	// Task 1: Start Frame acquisition.
		...
		for(int xx = 0; xx < N_MAX_STAGES; xx++){
			...
			{
				goodcenterX[xx*NB_MAX_DETECTION+yy] = 0;
				goodcenterY[xx*NB_MAX_DETECTION+yy] = 0;
				goodRadius[xx*NB_MAX_DETECTION+yy] = 0;

				goodcenterX_tmp[xx*NB_MAX_DETECTION+yy] = 0;
				goodcenterY_tmp[xx*NB_MAX_DETECTION+yy] = 0;
		...
		feature_scaled->tilted = 0;
		for (int kk = 0; kk<N_RECTANGLES_MAX; kk++)
		{
			feature_scaled->rect[kk].r.x0 = 0;
			feature_scaled->rect[kk].r.y0 = 0;
			feature_scaled->rect[kk].r.width = 0;
			feature_scaled->rect[kk].r.height = 0;
			feature_scaled->rect[kk].weight = 0.f;
		}
		...
		myQueue.memcpy(dev_feature_scaled, feature_scaled, sizeof(CvHaarFeature)); 

		myQueue.memcpy(dev_goodcenterX_tmp, goodcenterX_tmp, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(float)); 
		myQueue.memcpy(dev_goodcenterY_tmp, goodcenterY_tmp, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(float)); 
		myQueue.memcpy(dev_goodRadius_tmp, goodRadius_tmp, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(uint32_t)); 
        
        myQueue.memcpy(dev_goodcenterX, goodcenterX, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(uint32_t)); 
		myQueue.memcpy(dev_goodcenterY, goodcenterY, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(uint32_t)); 
		myQueue.memcpy(dev_goodRadius, goodRadius, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(uint32_t)); 

		myQueue.memcpy(dev_nb_obj_found2, nb_obj_found2, N_MAX_STAGES*sizeof (uint32_t)); 
        myQueue.memcpy(dev_nb_obj_found, nb_obj_found2, N_MAX_STAGES*sizeof (uint32_t)); 
        
        myQueue.memcpy(dev_scale_index_found, &scale_index_found, 1*sizeof(uint32_t)); 
		///////////////////////////////////////////////////////////////////

		TRACE_INFO(("Load Image Done %s!\n", imgName));
		printf("Load Image Done %s!\n", imgName);

		// Compute the Interal Image 

		//SYCL GPU 
		computeIntegralImgRowSYCL((uint32_t *)dev_img, (uint32_t *) dev_imgInt, width, height);
        computeIntegralImgColSYCL((uint32_t *)dev_imgInt, width, height);
              
		// Calculate the Image square 
		imgDotSquareSYCL((uint32_t *)dev_img, (uint32_t *)dev_imgSq, width, height);

		/* Compute the Integral Image square */
        computeIntegralImgRowSYCL((uint32_t *)dev_imgSq, (uint32_t *) dev_imgSqInt, width, height);
        computeIntegralImgColSYCL((uint32_t *)dev_imgSqInt, width, height);

		// Copy the Image to float array 

		imgCopySYCL((uint32_t *)dev_imgInt, (double *)dev_imgInt_f, height, width);
		imgCopySYCL((uint32_t *)dev_imgSqInt, (double *)dev_imgSqInt_f, height, width);

		///////////////// MEMCPY DEVICE TO HOST SYCL TTTTTTEEEEEEEEEESSSSSTTTTTTTTTT //////////////////////

		myQueue.memcpy(imgSqInt_f, dev_imgSqInt_f, width*height*sizeof(double)); 
		myQueue.memcpy(imgInt_f, dev_imgInt_f, width*height*sizeof(double));
	...
	// Task 2: Start Frame processing.
        ...
        unsigned int ts_0 = total_scales, rc = (irowiterations*icoliterations); 
                        ...
                        if(goodPoints_value){
                            for (int iStage = 0; iStage < nStages; iStage++)
                            {
                                ...
                                if (goodPoints_value)
                                {
                                    ...
                                    {
                                        computeFeature((double *)dev_imgInt_f, (double *)dev_imgSqInt_f, dev_cascade->stageClassifier[iStage].classifier[iNode].haarFeature,
        ...
        unsigned ts=total_scales;
         myQueue.submit([&] (sycl::handler& cgh){
            cgh.parallel_for(sycl::range<1>{ts},[=] (sycl::id<1> idx){
                int s = idx[0]; 
		...
		//////////////////////////// MEMCPY DTH SYCL ///////////////////////////////////////
        myQueue.memcpy(goodcenterX, dev_goodcenterX, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(uint32_t)); 
		myQueue.memcpy(goodcenterY, dev_goodcenterY, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(uint32_t)); 
		myQueue.memcpy(goodRadius, dev_goodRadius, NB_MAX_DETECTION*N_MAX_STAGES*sizeof(uint32_t)); 
        
        myQueue.memcpy(nb_obj_found2, dev_nb_obj_found2, N_MAX_STAGES*sizeof(uint32_t)); 
        myQueue.memcpy(&scale_index_found, dev_scale_index_found, 1*sizeof(uint32_t)); 
	...
	// Task 3: Start Frame post-processing.
		...
		for(i=0; i<scale_index_found; i++)
		{
			...
			if(nb_obj_found2[scale_index_found])
				scale_index++; 
		}
		...
		offset_X=(int)(real_width/(float)(scale_index*1.2));
		offset_Y=(int)(real_height/(float)(scale_index*1.2));

		// printf("Offsets [%d , %d] x: %d - y: %d\n", real_width, scale_index_found, offset_X, offset_Y); 
		...
		for(i=0; i<NB_MAX_POINTS; i+=3)
		{
			for(j=3; j<NB_MAX_POINTS-i; j+=3)
			{
				...
				{
					if(offset_X >= (int)fabs(position[i]-position[i+j]) && offset_Y >= (int)fabs(position[i+1]-position[i+j+1]))
					{
						position[i+j] = 0;
						position[i+j+1] = 0;
						position[i+j+2] = 0;
	...
	const float final = (double)(end-start)/CLOCKS_PER_SEC * 1000;
	printf("\nTOTAL Execution time = %f for %d frames -- %d fps ms.\n", final, (argc-2), (final/100)/(argc-2) );
